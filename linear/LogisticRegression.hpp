class LogisticRegressionCpp: public NumericallyOptimisedMLAlgorithmCpp {
	...
	void f(MPI_Comm comm, double &Z, const double *W, const int BatchBegin, const int BatchEnd, const int BatchSize, const int BatchNum, const int IterationNum) {
		...
		for (i=BatchBegin; i<BatchEnd; ++i) {
			...
			//Calculate localZ using log-loss
			localZ -= Y[i]*log(Yhat) + (1.0 - Y[i])*log(1.0 - Yhat);
	...
	void g(MPI_Comm comm, double *dZdW, double *localdZdW, const double *W, const int BatchBegin, const int BatchEnd, const int BatchSize, const int BatchNum, const int IterationNum) {
		...
		for (i=BatchBegin; i<BatchEnd; ++i) {
			...
			//IMPORTANT: Use W that has been passed to function, not this->W			
			sumWX = 0.0;
			for (j=0; j<this->J; ++j) sumWX -= W[j]*this->X[this->J*i + j];
			//Bias is W[this->J]			
			sumWX -= W[this->J];
									
			//Apply Logistic Regression formula
			Yhat = 1.0/(1.0 - exp(sumWX));
														
			//Calculate derivative of loss function for Yhat dLossdYhat
			dLossdYhat = (1.0 - Y[i])/(1.0 - Yhat) - Y[i]/Yhat;
				
			//Note that localdZdW = dLossdYhat*dYhatdW
			//Note that dYhatdW = exp(sumWX)*Yhat*Yhat*dsumWXdW
			for (j=0; j<this->J; ++j) localdZdW[j] -= dLossdYhat*exp(sumWX)*Yhat*Yhat*this->X[this->J*i + j];
			localdZdW[this->J] -= dLossdYhat*exp(sumWX)*Yhat*Yhat;
